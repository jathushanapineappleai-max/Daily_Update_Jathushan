...
import totalDaysIcon from '../../assets/icons/total_days.png';
import CheckoutConfirmModal from '../../modals/CheckoutConfirmModal';
import { clockIn, clockOut, startBreak, endBreak, getTodayAttendance, getAttendanceSummary } from '../../integration/attendanceAPI';
...
const TodayAttendance = () => {
          ...
          if (attendance.clock_in) {
            ...
            if (attendance.clock_out) {
              ...
              if (attendance.total_break_duration !== undefined) {
                const breakMins = Math.floor(attendance.total_break_duration / 60);
              ...
              if (attendance.total_break_duration !== undefined) {
                const totalBreakMins = Math.floor(attendance.total_break_duration / 60);
                const totalBreakSecs = attendance.total_break_duration % 60;
                setDisplayTime(prev => ({
                  ...prev,
                  breakTime: `${String(totalBreakMins).padStart(2, '0')}:${String(totalBreakSecs).padStart(2, '0')} mins`
                }));
              }
            } else {
              setAttendanceState('checkedIn');
  ...
  // Re-apply card animations when attendance state changes to ensure visibility
  useEffect(() => {
    // Ensure container remains visible
    if (containerRef.current && !containerRef.current.classList.contains('ta-container-enter')) {
      containerRef.current.classList.add('ta-container-enter');
    }
  ...
  if (loading && !checkInTime && attendanceState === 'initial') { // Only show loading when initializing
    return (
      <div className="today-attendance-container" ref={containerRef}>
        <h2 className="ta-title">Today Attendance</h2>
  ...
  return (
    <div className="today-attendance-container" ref={containerRef}>
      ...
      {error && (
        <div className="error-container" style={{ padding: '10px', textAlign: 'center', color: 'red', backgroundColor: '#ffe6e6', margin: '10px', borderRadius: '4px' }}>
          <p>Error: {error}</p>
        </div>
      )}
      ...
      <div className="ta-actions">
        <button
          className={`ta-btn-break ${attendanceState === 'checkedIn' ? 'active' : ''} ${attendanceState === 'checkedOut' ? 'disabled-after-checkout' : ''}`}
          onClick={attendanceState === 'onBreak' ? handleBackToWork : handleTakeBreak}
          disabled={attendanceState === 'initial' || attendanceState === 'checkedOut' || loading}
        >
          {loading ? 'Processing...' : (attendanceState === 'onBreak' ? 'Back to Work' : 'Take a Break')}
        </button>
        <button
          className={`ta-btn-checkout ${attendanceState === 'onBreak' ? 'error' : ''} ${attendanceState === 'checkedOut' ? 'disabled-after-checkout' : ''}`}
          onClick={attendanceState === 'initial' ? handleCheckIn : handleCheckOutClick}
          disabled={attendanceState === 'onBreak' || attendanceState === 'checkedOut' || loading}
        >
          {loading ? 'Processing...' : (attendanceState === 'initial' ? 'Check In' : 'Check Out')}
        </button>
      </div>
